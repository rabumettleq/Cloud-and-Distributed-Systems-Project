...
def createSpark(workers: int):
    return (
        SparkSession.builder
        .appName(f"ML-Performance-Test-{workers}")
...
def run_regression_job(df: DataFrame) -> dict:
    numericCols = _getNumericCols(df)

    if len(numericCols) < 2:
        return {"jobName": "regression", "skipped": True, "reason": "Not enough numeric columns (need at least 2: label + feature)."}
    ...
    if regDf.count() == 0:
        return {"jobName": "regression", "skipped": True, "reason": "No rows after removing nulls."}
...
def run_classification_job(df: DataFrame) -> dict:
    numericCols = _getNumericCols(df)
    labelCandidates = _getCandidateLabelCols(df)

    if len(numericCols) < 1:
        return {"jobName": "classification", "skipped": True, "reason": "No numeric feature columns found."}

    if len(labelCandidates) == 0:
        return {"jobName": "classification", "skipped": True, "reason": "No suitable label column found (need a column with 2..20 distinct values)."}
    ...
    if len(featureCols) == 0:
        return {"jobName": "classification", "skipped": True, "reason": "No numeric features left after excluding label column."}
    ...
    if assembledDf.count() == 0:
        return {"jobName": "classification", "skipped": True, "reason": "No rows after removing nulls/invalid labels."}

    trainDf, testDf = assembledDf.randomSplit([0.8, 0.2], seed=42)
    if trainDf.count() == 0 or testDf.count() == 0:
        return {"jobName": "classification", "skipped": True, "reason": "Not enough data after train/test split."}
...
def run_clustering_job(df: DataFrame) -> dict:
    numericCols = _getNumericCols(df)

    if len(numericCols) < 2:
        return {"jobName": "clustering", "skipped": True, "reason": "Not enough numeric columns (need at least 2)."}

    vecDf = _prepareNumericFeatures(df, numericCols).select("features")
    if vecDf.count() == 0:
        return {"jobName": "clustering", "skipped": True, "reason": "No rows after removing nulls."}
...
def run_timeseries_job(df: DataFrame) -> dict:
    numericCols = _getNumericCols(df)
    dateCol = _findDateColumn(df)

    if dateCol == "":
        return {"jobName": "timeSeries", "skipped": True, "reason": "No date/time column found (try naming it date/time/timestamp)."}

    if len(numericCols) == 0:
        return {"jobName": "timeSeries", "skipped": True, "reason": "No numeric columns found for aggregation."}
    ...
    if tsDf.count() == 0:
        return {"jobName": "timeSeries", "skipped": True, "reason": "No rows after parsing timestamps or removing nulls."}
...
def _bench_one_job(job_fn, df: DataFrame):
    start = time.time()
    result = job_fn(df)
    end = time.time()
...
def _bench_workers_for_job(filePath: str, fileType: str, job_fn):
    rows = []
    baseline = None

    for w in WORKERS_LIST:
        spark = createSpark(w)
        df = loadDataset(spark, filePath, fileType)

        df.cache()
        df.count()

        job_result, elapsed = _bench_one_job(job_fn, df)
        ...
        rows.append({
            ...
            "skipped": bool(job_result.get("skipped", False)),
...
def runPerformanceTestPerJob(filePath: str, fileType: str):
    ...
    if fileType == "text":
        return {
            ...
            "clustering": [],
            "timeSeries": [],
            "note": "Text data is not supported for these ML jobs."
        }

    return {
        "regression": _bench_workers_for_job(filePath, fileType, run_regression_job),
        "classification": _bench_workers_for_job(filePath, fileType, run_classification_job),
        "clustering": _bench_workers_for_job(filePath, fileType, run_clustering_job),
        "timeSeries": _bench_workers_for_job(filePath, fileType, run_timeseries_job),
    }
